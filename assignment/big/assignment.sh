...
rm results.txt &> /dev/null #Removes old file and pipes error into null if it din't exist
...
rangeFiles(){ #Downloads a range of files
    ...
    read -p "What is the start of the range you wish to download? " lower
    inputCheck $lower
    lower=$correctInput #Assigns lower file name

    read -p "What is the end of the range you wish to download? " upper
    inputCheck $upper
    upper=$correctInput
...
until [[ $option = "x" ]]; do #Loops until user chooses to exit program
    ...
    if [ $option = "1" ]; then
        ...
        echo "Please enter either 1, 2, 3, 4 or x only."
